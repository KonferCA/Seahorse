...
let DataProviderContract = (_dec = NearBindgen({}), _dec2 = initialize(), _dec3 = call({}), _dec4 = call({}), _dec5 = call({}), _dec6 = call({}), _dec7 = call({}), _dec8 = view(), _dec9 = view(), _dec(_class = (_class2 = class DataProviderContract {
  ...
  add_provider({
    ...
    log(`Adding provider: ${id}, name: ${name}, valueScore: ${valueScore}, wallet: ${walletAddress}`);
    if (valueScore < 1 || valueScore > 100) {
      log(`Invalid value score: ${valueScore}`);
    ...
    log(`Successfully added provider ${id}`);
  }
  update_provider_value({
    id,
    valueScore
  }) {
    log(`Updating provider ${id} value score to ${valueScore}`);
    if (valueScore < 1 || valueScore > 100) {
      log(`Invalid value score: ${valueScore}`);
      throw new Error("Value score must be between 1 and 100");
    }
    ...
    if (!provider) {
      log(`Provider ${id} not found`);
      throw new Error("Provider not found");
    }
    provider.valueScore = valueScore;
    this.providers.set(id, provider);
    log(`Successfully updated provider ${id} value score`);
  }
  add_provider_data({
    providerId,
    data
  }) {
    log(`Starting add_provider_data for provider: ${providerId}`);
    log(`Number of items to add: ${data.length}`);
    const providerDataVector = this.providerData.get(providerId);
    if (!providerDataVector) {
      log(`Provider ${providerId} not found`);
      throw new Error("Provider not found");
    }
    const newVector = new Vector(`v_${providerId}`);
    for (const item of data) {
      log(`Adding item: ${item.id}, content length: ${item.content.length}`);
      newVector.push(item);
    }
    this.providerData.set(providerId, newVector);
    log(`Successfully added ${data.length} items for provider ${providerId}`);
  }
  remove_provider_data({
    providerId,
    dataIds
  }) {
    log(`Starting remove_provider_data for provider: ${providerId}`);
    log(`Items to remove: ${dataIds.join(', ')}`);
    const providerDataVector = this.providerData.get(providerId);
    if (!providerDataVector) {
      log(`Provider ${providerId} not found`);
      throw new Error("Provider not found");
    ...
    log(`Items remaining after filter: ${newData.length}`);
    const newVector = new Vector(`v_${providerId}`);
    for (const item of newData) {
      log(`Keeping item: ${item.id}`);
      newVector.push(item);
    }
    this.providerData.set(providerId, newVector);
    log(`Successfully removed specified items for provider ${providerId}`);
  }
  process_query({
    queryResults
  }) {
    log(`Starting process_query with ${queryResults.length} results`);
    const payouts = {};
    for (const result of queryResults) {
      log(`Processing result for provider: ${result.providerId}`);
      log(`Relevancy score: ${result.relevancyScore}`);
      const provider = this.providers.get(result.providerId);
      if (!provider) {
        log(`Provider ${result.providerId} not found, skipping`);
        continue;
      }
      log(`Provider ${provider.id} found, value score: ${provider.valueScore}`);
      const payout = BigInt(provider.valueScore) * BigInt(result.relevancyScore) * BigInt(1e17);
      log(`Calculated payout: ${payout} yoctoNEAR`);
      payouts[provider.walletAddress] = (payouts[provider.walletAddress] || BigInt(0)) + payout;
      log(`Updated total payout for ${provider.walletAddress}: ${payouts[provider.walletAddress]} yoctoNEAR`);
    }
    for (const [walletAddress, payout] of Object.entries(payouts)) {
      log(`Processing payout for wallet: ${walletAddress}`);
      log(`Attempting to transfer ${payout} yoctoNEAR to ${walletAddress}`);
      const promiseIndex = promiseBatchCreate(walletAddress);
      promiseBatchActionTransfer(promiseIndex, payout);
      log(`Transfer initiated for ${walletAddress}`);
    }
    log('Query processing completed');
  }
  get_provider({
    id
  }) {
  ...
  get_provider_data({
    ...
    log(`Getting data for provider: ${providerId}`);
    const providerDataVector = this.providerData.get(providerId);
    if (providerDataVector) {
      const data = providerDataVector.toArray();
      log(`Found ${data.length} items for provider ${providerId}`);
      return data;
    } else {
      log(`No data found for provider ${providerId}`);
      return [];
    }
  }
...
function get_provider_data() {
  ...
  const _result = _contract.get_provider_data(_args);
...
function get_provider() {
  ...
  const _args = DataProviderContract._getArgs();
  const _result = _contract.get_provider(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DataProviderContract._serialize(_result, true));
}
function process_query() {
  ...
  const _args = DataProviderContract._getArgs();
  const _result = _contract.process_query(_args);
  DataProviderContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DataProviderContract._serialize(_result, true));
}
function remove_provider_data() {
  const _state = DataProviderContract._getState();
  if (!_state && DataProviderContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = DataProviderContract._getArgs();
  const _result = _contract.remove_provider_data(_args);
  DataProviderContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DataProviderContract._serialize(_result, true));
}
function add_provider_data() {
  const _state = DataProviderContract._getState();
  if (!_state && DataProviderContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = DataProviderContract._getArgs();
  const _result = _contract.add_provider_data(_args);
  DataProviderContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DataProviderContract._serialize(_result, true));
}
function update_provider_value() {
  const _state = DataProviderContract._getState();
  if (!_state && DataProviderContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = DataProviderContract._getArgs();
  const _result = _contract.update_provider_value(_args);
  DataProviderContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DataProviderContract._serialize(_result, true));
}
function add_provider() {
  ...
  const _args = DataProviderContract._getArgs();
  const _result = _contract.add_provider(_args);
  DataProviderContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DataProviderContract._serialize(_result, true));
}
function init() {
